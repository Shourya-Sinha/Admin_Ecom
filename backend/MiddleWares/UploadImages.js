...
const uploadAvatar = async (file, existingPublicId) => {
    try {
      ...
      //console.log('Existing Public ID:', existingPublicId);
  
      // If an existing image is present, destroy it before uploading the new one
      if (existingPublicId) {
        //console.log('Attempting to delete image with public_id:', existingPublicId);
        const deleteResponse = await cloudinary.uploader.destroy(existingPublicId);
       // console.log('Delete Response:', deleteResponse);
        if (deleteResponse.result !== 'ok') {
          //console.error(`Failed to delete image with public_id ${existingPublicId}. Response:`, deleteResponse);
  ...
  const uploadImage = async (file, existingPublicId) => {
    try {
      ...
      console.log('catch uplaod error',error);
        throw error;
    }
};
  ...
  //     // Log the existing public ID
  //     //console.log('Existing Public ID:', existingPublicId);
  //        console.log('/files in uploadeProductImages',file);
  //     // If an existing image is present, destroy it before uploading the new one
  //     if (existingPublicId) {
  //       //console.log('Attempting to delete image with public_id:', existingPublicId);
  //       const deleteResponse = await cloudinary.uploader.destroy(existingPublicId);
