...
const loginUser = async (req, res, next) => {
  try {
    ...
    if (existingSession) {
      ...
      console.log("Updated token", existingSession.token);
...
//     //console.log('Decoded ID:', decoded.userId);;  // Verify the ID is extracted
...
const protect = async (req, res, next) => {
  try {
    ...
    if (decoded.exp < now) {
      //console.log("Token has expired", decoded.exp);
      return next(
        new AppError(401, "error", "Token has expired. Please log in again.")
      );
    }
    ...
    if (!user) {
      return next(
        new AppError(
          404,
          "error",
          "Invalid or expired resetToken send Email Again."
        )
      );
    }
    // 3 update password
...
const invalidateSessions = async (user) => {
  try {
    ...
    console.log("All sessions for user have been invalidated.");
  } catch (error) {
...
const updateCartItemQuantity = async (req, res, next) => {
  try {
    const userId = req.userId;
    const { productId, selectedColour, selectedVariant, newQuantity } =
      req.body;

    if (newQuantity < 1) {
      return next(new AppError(400, "error", "Quantity must be at least 1."));
    }
    ...
    // Update discount and discountedPrice
    if (cart.coupon && cart.coupon.code) {
      const totalPrice = parseFloat(cart.totalPrice.toString()); // Convert Decimal128 to number
      const discountAmount = (totalPrice * cart.coupon.discount) / 100;
      cart.discountAmount = mongoose.Types.Decimal128.fromString(
        discountAmount.toFixed(2)
      );
...
//     // Validate cart items to ensure all required fields are present
//     const validatedItems = cart.items.map(item => {
//       if (!item.selectedColour || !item.product || !item.quantity || !item.price) {
//         console.log("Missing fields in item:", item);
...
//         transactionId = "COD-" + new Date().getTime(); // Generate a unique ID for COD
//         break;
//       case "CreditCard":
//         paymentStatus = await processCreditCardPayment(totalPaid);
//         transactionId = "CC-" + new Date().getTime(); // Example transaction ID
//         break;
//       case "UPI":
//         paymentStatus = await processUPIPayment(totalPaid);
//         transactionId = "UPI-" + new Date().getTime(); // Example transaction ID
//         break;
//       case "PayPal":
//         paymentStatus = await processPayPalPayment(totalPaid);
//         transactionId = "PP-" + new Date().getTime(); // Example transaction ID
//         break;
//       default:
...
//       transactionId,
//       paymentDetails: {
//         // Add more details depending on the payment method
//       }
//     });
...
//       selectedAddress, // Assuming the address ID is provided
//       shippingMethod,
//       paymentMethod,
//       paymentInfo: payment._id, // Link to payment record
...
const createOrder = async (req, res, next) => {
  try {
    ...
    // Create Order first to validate everything before creating Payment
    const order = new Order({
      orderBy: userId,
      items: validatedItems, // Use validated items
      totalPrice: mongoose.Types.Decimal128.fromString(totalProductPrice.toFixed(2)),
      ...
      paymentInfo: null, // Will update after payment creation
      coupon: cart.coupon ? cart.coupon : undefined ,// Conditional coupon assignment
...
//     const { orderId, deliveryStatus, paymentStatus } = req.body;
...
//       // If delivered, set deliveredAt and return deadline
...
//         order.paymentStatus = paymentStatus;
//       } else {
//         return next(new AppError(400, "error", "Payment status can only be updated to 'Paid' for non-COD methods or any status for COD."));
//       }
//     }
...
//   try {
//     // Step 1: Get the admin's details
//     const adminId = req.user._id; // Assuming you have admin's ID from auth middleware
...
//     // Get today's date and previous month
//     const today = new Date();
//     const previousMonth = new Date(today.getFullYear(), today.getMonth() - 1, 1);
//     const nextMonth = new Date(today.getFullYear(), today.getMonth(), 1);

//     // Step 2: Get product statistics and totals
...
//       {
//         $facet: {
//           stats: [{ $sort: { totalRevenue: -1 } }], // Optional: Sort by revenue descending
//           totals: [
//             {
//               $group: {
//                 _id: null,
...
//     // Step 3: Get revenue of the previous month
//     const previousMonthRevenue = await Order.aggregate([
//       { $match: { createdAt: { $gte: previousMonth, $lt: nextMonth } } }, // Filter by previous month
//       { $unwind: "$items" },
//       {
//         $group: {
//           _id: null,
...
//       totalSold: totalSold,
//       previousMonthRevenue: previousMonthTotalRevenue,
